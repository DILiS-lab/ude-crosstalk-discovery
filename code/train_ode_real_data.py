...
from utils.neural_networks import ConstantNN
...
if isinstance(config["seed"], list):
    if len(sys.argv) == 2:  # Supervisor Mode
        ...
        # Use 'ode_' prefix since we are technically training just the ODE part
        batch_dir_name = f"ode_{config['model_name']}_uncertainty_{datetime.now().strftime('%Y-%m-%d_%H%M%S')}"
        ...
        experiment_name = f"ode_{config['model_name']}_real_data_{datetime.now().strftime('%Y-%m-%d_%H%M%S')}"
        experiment_folder = Path(project_root) / "experiments" / experiment_name
else:  # Standard Mode
    experiment_name = (
        f"ode_{config['model_name']}_real_data_{datetime.now().strftime('%Y-%m-%d_%H%M%S')}"
    )
    experiment_folder = Path(project_root) / "experiments" / experiment_name
...
print("Starting the training of the UDE model (Constant Crosstalk = No Crosstalk)")
...
# We don't really need to save the NN params since it's constant, but good to have consistency
...
print("Generating p53 predictions with trained ODE model.")
...
plot_data(
    ...
    true_p53_values,
    experiment_folder,
    "True vs Predicted Values of p53 levels (ODE only)",
    "Time [h]",
    "P53 level",
...
print("Check: Ensure crosstalk factor is constant 1.0.")
...
# Skip symbolic regression since we know the function is 1.0
