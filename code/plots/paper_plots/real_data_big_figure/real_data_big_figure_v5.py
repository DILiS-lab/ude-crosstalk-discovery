...
SR_FUNCS = {
    "hunziker": (r"$0.274\,[NF\text{-}\kappa B] + 0.717$",
                 lambda x: 0.2743812 * x + 0.7170906),
    "konrath":  (r"$[NF\text{-}\kappa B]^2 + 1.698$",
...
# Figure
# ---------------------------------------------------------------------------
COL_WIDTHS = [0.5, 0.5, 0.765, 0.583]

fig = plt.figure(figsize=(7.00787, 2.5), layout="constrained")
...
gs_col2 = gs[0, 2].subgridspec(3, 2, hspace=0.001, wspace=0.001)
ax_col2 = []
ax_col2_ref = None
for r in range(3):
    row = []
...
for dataset, ax in zip(DATASETS_BY_ROW, ax_cv):
    ...
    ax.set_ylabel("Time-averaged rMAE")
    legend_handles = [
...
ax_col2[0][1].axis("off")
handles, labels = ax_col2[0][0].get_legend_handles_labels()
ax_col2[0][1].legend(handles, labels, loc="center", ncol=1, fontsize=6)

# ---------------------------------------------------------------------------
# Axis labels
# ---------------------------------------------------------------------------
# Col 0 (CV)
ax_cv[1].set_xlabel("Dataset Split")

# Col 1 (RMAE)
ax_col1_r0.set_ylabel("Cell-averaged rMAE")
ax_col1_r1.set_ylabel("Cell-averaged rMAE")
...
plt.savefig(SCRIPT_DIR / "real_data_big_figure_v5.pdf", dpi=300, bbox_inches="tight")
# plt.show()
